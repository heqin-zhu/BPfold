...
@timer
def evaluate(dest_path:str, pred_dir:str, data_dir:str, pairwise:bool=True, read_pred=None, testsets=None, show_nc=False)->None:
    '''
    Cal metrics of predicted SS in pred_dir and gt SS in data_dir, then save results in dest_path (.yaml).
    ...
    metric_dic = {}
    missing_pred = {}
    index_path = os.path.join(data_dir, 'data_index.yaml')
    index_data = {}
    for dic in read_yaml(index_path)['test']:
        dataset = dic['dataset']
        if dataset not in index_data:
            index_data[dataset] = []
        index_data[dataset].append(os.path.join(data_dir, dic['path']))
    for dataset, gt_files in index_data.items():
...
def summary(path, to_latex=True):
    ...
    for tag, cur_dic in [('len>600', metric_dic_gt600), ('len<=600', metric_dic_le600), ('all', metric_dic)]:
        title_row = format_row(['dataset', 'num', 'f1', 'p', 'r'], [15, 5, 5, 5, 5])
        ...
        for dataset_name, dic in cur_dic.items():
            if len(dic)!=0:
                ...
                outputs.append(format_row(items, [15, 5, 5, 5, 5]))
...
if __name__ == '__main__':
    ...
    if not args.testsets:
        args.testsets = "archiveII bpRNA Rfam14.5-14.10 PDB_test".split()
    if args.pred_dir:
        if args.tag is None:
        ...
        evaluate(dest_path, args.pred_dir, args.data_dir, testsets=args.testsets, show_nc=args.show_nc)
        summary(dest_path)
    elif args.summary:
